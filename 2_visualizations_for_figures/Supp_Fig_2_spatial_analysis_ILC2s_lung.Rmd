...
```{r, fig.width=9, fig.height=6}
ggarrange(plotlist = g_master_list[c(9, 8, 7, 6, 3, 2, 1)], ncol = 4, nrow = 2, 
          labels = "AUTO")
```

## Coenrichment ILC2s & myeloid cells
...
plot_if_myeloid <- plot +
  annotate("text", x=90, y=970, label= marker1,
           col="cyan", size=5, parse=TRUE) +
  annotate("text", x=235, y=970, label= marker2,
           col="magenta", size=5, parse=TRUE) +
  annotate("text", x=400, y=970, label= marker3,
           col="yellow", size=5, parse=TRUE)+
  annotate("text", x=550, y=970, label= marker6,
...
## ILC2s on overlay of immune cells
...
## ILC2s plotted on ILC marker overlay
...
## Proportion around immune cells
...
df_dist_ref <- df_dist_all %>%
  ...
  filter(RefType == "ILC2s"
         |
           RefType == "NK cells/ILC1s"| RefType == "ILC3s"|
           RefType == "T helper cells"| RefType == "T cytotox cells"| RefType == "B cells & Plasma cells"
         )
...
for (condition in c("CTRL", "1", "2", "3")) {
  ...
  # create plot
  plot <- ggboxplot(plot_data, x="RefType" , 
            y = "Distance", 
            fill = "RefType",
            palette = ColorsCellType,
            outlier.shape = NA, 
            width = 0.7,
            # facet.by = "NearestType",
            font.label = list(size = 16, color = "black"),
            short.panel.labs = FALSE) +
    geom_boxplot(alpha = 0.3, lwd=1, width = 0.7, outlier.shape = NA)+
    stat_pvalue_manual(stat.test,
                       size = 6,
                       hide.ns = TRUE, y.position = 250, step.increase = 0.1
                       )+
    theme_classic2()+
    ggtitle(gsub("3", "D3", condition))+
    xlab("Cell types")+
    ylab("Minimal distance [µm]")+
    scale_y_continuous(expand = c(0, 0), limits = c(0,400))+
    rotate_x_text(angle = 45)+
...
dist_myloid <- ggarrange(plotlist = my_plot_list[c(1, 4)], nrow = 1, ncol = 2)
dist_myloid
...
df_dist_ref <- df_dist_all %>%
  # filter(Treatment == condition) %>%
  filter(RefType == "Myeloid cells" |RefType == "ILC2s"|
           RefType == "NK cells/ILC1s"| RefType == "ILC3s"|
           RefType == "T helper cells"| RefType == "T cytotox cells"| RefType == "B cells & Plasma cells"
         )
...
dist_ilc2s <- ggarrange(plotlist = my_plot_list[c(1, 4)], nrow = 1, ncol = 2)
dist_ilc2s
```
